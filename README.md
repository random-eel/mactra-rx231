...
設定ツール説明等は [こちら](https://github.com/random-eel/mactra-rx231/wiki)  
...
※~~現在ファームウェア開発中 開発状況を以下に示します。~~ 26/2/24 最低限の機能を実装したため、ファームウェア開発を完了しました。   
|開発状況|内容|説明|
|:---:|:---:|---:|
|✅️|Lチカ|背面LED点滅|
...
書き込み用スイッチ(横のスライドスイッチ)がオンのままGNDに接続しないで下さい。  
...
1. **電源を切った状態**で、背面ディップスイッチを BOOT MODE: **Off**; USB BOOT: **Off**; に設定します。
2. **電源を切った状態**で、E2エミュレータとキーボードを接続します。  
3. CS+/e2studio で E2エミュレータの設定をし、接続します。 (エミュレータから電源供給可 - LED使用時は注意)
4. プログラムを実行します。  
※ E2エミュレータから電源を供給すると、5V端子の出力は無効化されます。  
※ E2エミュレータから電源を供給しても、RGB LEDにも3.3v電源が供給されます。供給電流を超えないように明るさを調整して下さい。  
</details>

> ##### ⚠️ Rev.1 & 2の場合
> GPIO P16ピン と GND ピンを接続しないでください。  
> 電源がショートし、最悪破損します。  
...
## ケースについて
> [!IMPORTANT]
> 26/3/1 時点でファイルが更新されておりません。 今暫くお待ち下さい。
  
casesフォルダに3Dプリンタで出力可能なモデルを用意してあります。  
以下のネジが必要になります。何れもセルフタップになります。  
 - 3Dプリントしたケースの場合 (Type 1: 中間+キーカバー)
   - M2 15mmネジ x 4
 - 3Dプリントしたケースの場合 (Type 2: 中間のみ)
    - M2 5mmネジ x 8
...
## ソースコードについて (TBD)
Renesas製のサンプルコード及びFITモジュールを使用しているため、公開可能かどうか不明です。(もう少し調べます...)  
  
御連絡頂ければソースコードをお送り致しますが、最低でもルネサスのアカウントを取得してください。    
(使用したコード・モジュール全てが、ルネサスのアカウントさえあればダウンロード可能な為)  
  
> [!WARNING]
> #### ⚠️ AI生成コードについて
> このキーボードのファームウェア開発にはGeminiを使用し生成したコードが含まれています。ご了承下さい。  
...
* [dsa-keycap](https://github.com/anhthang/dsa-keycap)
* ポリテクセンターの講義・資料
* Google Gemini
...
### RGB LEDの設定を変更したい
ロータリーエンコーダを押し込みながらSW1(ロータリーエンコーダ下スイッチ)を押すと、設定変更モードになります。  
上下で設定項目の変更、回転で数値を変更します。    
LEDが黄色の状態で左右を入力すると、LEDの点灯モードが変更できます。
|色|機能|
|---|---|
|黄|輝度変更|
|赤緑青|色彩変更|
...
### スマホからキーマップ変更出来ない  
現状WebHIDのサポートが無いようです。(Android/iPhone)  
設定変更にはPCを使用して下さい。  
### キーボードのリセットを行いたい
ロータリーエンコーダを押し込みながらSW1(ロータリーエンコーダ下スイッチ)を押したまま、USBケーブルを繋げて下さい。  
それでもリセットされない場合、エンコーダ押し込み+上部のキー全てを押しながら、ケーブルを繋げて下さい。  
### 基板から注文したい
...
### なぜ製作に至ったのか 
職業訓練校で使い方を学んだRXシリーズを使用して、自身のスキルを試し、能力を証明してみたかった点が一番の理由です。  
RXシリーズの開発ボードを勉強目的で安価に手に入れたかったのですが、普通の物では面白くないと思い、制作に至りました。
  
## 免責事項
このデバイス 及び ファームウェアを使用していて、誤字が増えた、ホストデバイスが壊れた、核戦争が起きた、デバイスが正しく動作しなかったせいで会社をクビになった等があっても、設計/製作者は一切の責任を負いません。  
  
## その他
 
<details>
  <summary>読まなくてもいい詳細</summary>
  
<details>
  <summary>本当に読むんですか？ (どうでもいい事しか書いてないです) </summary>

<details>
  <summary>本当の本当に読むんですか？ (実質知り合い向けな上、色々と遠慮しておりません) </summary>

<details>
  <summary>本当の本当の本当に読むんですか？ (NOT WORTH READING THIS (mostly), YOU HAVE BEEN WARNED) </summary>
...
( ◠‿◠ ) 回路図あるんだから自分で用意しろ！  
### 他のMCUで作る予定は？
( ◠‿◠ ) 無い。（多分… ルネサスのチップを載せたくなったら作ります）  
### でも他のMCUの方が安いし楽なのでは?
RXシリーズの勉強用なので、それでは意味が無かったんです。  
### 勉強に使える開発ボードにしてはBYTEでアクセスしづらくない？  
安く手に入るチップがこれだったので仕方がありませんでした。  
もし次にルネサス製のチップを使うならRAにします。  
### 設定用データのパケットの内容は？
設定ツールを見て下さい。(そのうち説明は追加します)  
### 回路図は？  
...
### 基板データは？
いずれ追加予定です。（Rev.2も思った通りの動作ではない為未定ですが、近日追加します)   
### BOMは？
同梱予定です。少量の位置確認が必要だと思われます。  
スイッチソケット・ダイオードアレイ・TVSダイオード・USBコネクタの位置確認が必要です。  
### アナログ入出力ヤバいんだけど  
...
スイッチのグラウンド不良の可能性がありますが、不明です。(シールド実測で0.3v程 スイッチのNCを真に受けた為?)  
### 配線汚いし間違ってるとかアホ？
( ◠‿◠ ) うるせえ 初心者なんだよ 誰もが通る道なんだよ
### AI使うとか雑魚？
雑魚です。正直かなり助かりました。特にUSB周りはヘルプ無しでは無理だったと思います。  
### FAQ舐めてんの？
仲間・友人程度しか見ないだろうと踏んでおります。  
あくまで勉強のための趣味の品だと思って頂けると幸いです。  
こんな素人の作品に本気になってどーすんのｗ   
### 設計者は何奴？
よくわからん少しキーボード好きの人間です。普段はRP2040で制作しています。どうでもいいですが求職中です。(26/3/1)
</details>
</details>
</details>
</details>
